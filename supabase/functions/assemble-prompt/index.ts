...
// ========== PROMPT TEMPLATES (server-side only) ==========
const PROMPT_TEMPLATES: Record<string, { template: string }> = {
  "1A": {
...
function buildContextChainBlock(chain: ContextChainEntry[]): string {
  ...
  const lines = [
    ...
    ...chain.map((c) => `[${c.moduleId} – ${c.moduleName}]: ${c.summary}`),
    `--- END CONTEXT CHAIN ---`,
  ];
  return lines.join("\n");
}
...
function assemblePrompt(
  ...
  const templateObj = PROMPT_TEMPLATES[moduleId];
  ...
  const ccBlock = buildContextChainBlock(contextChain);
  if (ccBlock) {
    prompt = `${prompt}\n${ccBlock}`;
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const { moduleId, formData, designPassport, contextChain, platformType } = body;

    if (!moduleId || !formData) {
      return new Response(JSON.stringify({ error: "Missing moduleId or formData" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const prompt = assemblePrompt(
      moduleId,
      formData,
      designPassport || { mood: "", references: [], primaryColor: "#6C5CE7", secondaryColor: "#00F5D4", fontPreference: "" },
      contextChain || [],
      platformType || "web"
    );
