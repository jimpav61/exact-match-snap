...
// PRD section definitions — each maps to one of the 9 Karpathy-style templates
const PRD_SECTIONS = [
...
// Templates (server-side only — same as assemble-prompt)
...
function assembleSection(
  ...
  // Add context chain from previously generated sections
...
serve(async (req) => {
  ...
  try {
    ...
    // Generate all 9 sections with context chain
    const sections: { id: string; name: string; phase: string; content: string }[] = [];

    for (const section of PRD_SECTIONS) {
      const content = assembleSection(
        section.id,
        userInput,
        dp,
        platform,
        sections.map((s) => ({ id: s.id, name: s.name, content: s.content }))
      );
    ...
    // Save all sections as module_responses
    ...
    await supabase
      .from("projects")
      .update({ current_phase: 3, current_module: "3C" })
      .eq("id", projectId);
